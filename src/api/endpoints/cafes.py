...
from src.api.validators import check_duplicate_cafe, handler_cafe_error
...
async def get_cafes(
    ...
    if user.is_admin() and show_all:
        return await handler_cafe_error(
...
async def get_cafe(
    ...
    if user.is_admin():
        obj_db = await handler_cafe_error(
            cafe_crud.get,
