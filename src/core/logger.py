...
def init_logger(settings: Any) -> None:
    ...
    formatter = logging.Formatter(
        fmt=(
            ...
            '%(username)s | %(user_id)s | %(message)s'
        ),
        datefmt='%Y-%m-%d %H:%M:%S',
    )
...
def project_log(
    ...
    """Универсальный логгер для всего проекта."""
    username = (
        getattr(user, 'username', SYSTEM_USERNAME) if user else SYSTEM_USERNAME
    )
    user_id = getattr(user, 'id', DEFAULT_USER_ID) if user else DEFAULT_USER_ID
    ...
    logger.log(getattr(logging, level.upper()), full_message, extra=extra)


def log_endpoint(level: str = 'info') -> Callable:
    """Декоратор для логирования вызовов эндпоинтов FastAPI."""
    ...
    def decorator(
        ...
        async def wrapper(*args: Any, **kwargs: Any) -> Any:
            ...
            project_log(level, f'Вызов эндпоинта: {func.__name__}', user=user)
